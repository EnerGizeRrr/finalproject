...
class CommentController extends Controller
{
     ...
     */
    public function store(StoreCommentRequest $request, Task $task) // $task уже внедрен из маршрута
    {
        // Передаем $task в authorize
        $this->authorize('create', [Comment::class, $task]);

        $comment = $task->comments()->create(array_merge(
            $request->validated(),
            ['user_id' => Auth::id()] // Устанавливаем ID текущего пользователя
        ));
