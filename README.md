...
2.  **Установите зависимости Composer (внутри контейнера Laravel):**
    ```bash
...
6.  **(Опционально) Запустите seeders для начальных данных:**
    ```bash
    docker-compose exec laravel.test php artisan db:seed
    ```

7.  **Запустите воркер очередей в отдельном терминале:**
    ```bash
    docker-compose exec laravel.test php artisan queue:work
    ```

8.  **API будет доступен по адресу:**
...
*   **Запуск тестов:**
    ...
    ```
    > **Важно:** Некоторые тесты (например, `ProcessWebhookRequestQueueTest`) требуют, чтобы воркер очередей `queue:work` был запущен вручную в отдельном терминале с тестовым окружением (`--env=testing`) для корректной проверки обработки задач и попадания их в `failed_jobs`.
