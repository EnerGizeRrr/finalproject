...
COPY docker/init_app.sh /usr/local/bin/init_app.sh

RUN chmod +x /usr/local/bin/init_app.sh
...
RUN apt-get update \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y gnupg gosu curl ca-certificates zip unzip git supervisor sqlite3 libcap2-bin libpng-dev python3 python3-dev python3-pip python3-setuptools gcc g++ pkg-config make nano vim jq \
